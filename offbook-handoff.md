...
### Step 1 — De-risk before building (cheap, may change scope; items 1 & 1a are hard gates)
1. **WS-fidelity spike (hard gate on "v1 done," §12.1) — split into two halves so the doable half isn't blocked by the air-gapped one.** (a) *Doable now, in any environment:* an `mqtt.js` client (close-enough stand-in for the SPA) connects to a bare Aedes ws listener, subscribes, and receives a **retained** message — this becomes the **M0 gate** (build-plan §3). (b) *Air-gapped homework:* confirm the **proprietary-fork** WS handshake (protocol level, path, subprotocol, auth shape) matches, using the real app's captured `connect()` (item 2). Half (a) proves Aedes speaks browser-MQTT-over-ws; only (b) proves parity with prod. (§3, §12.1)
1a. **Real-spec-corpus audit (hard gate; air-gapped homework, §12.1a).** Pull the actual AsyncAPI specs the browser app consumes; grade each channel against §7 Modes 1/2/3 (loads? vacuous? fresh?) **and** extract the org's MQTT-binding/transport conventions (every transport fixture is hand-authored — no public MQTT-over-WS idiom exists — so the fixtures encode assumptions only this audit confirms). Strong corpus ⇒ validation's value is real; weak ⇒ first value is upstream spec hygiene and validation stays tenuous (design §1). (§7, §12.1a)
...
**Gate:** the **doable half** of item 1 (Aedes⟷`mqtt.js` retained receipt) is folded into the **M0 gate** and built first (build-plan §3). The **air-gapped halves** — item 1(b) fork parity and item 1a corpus audit — are **hard gates on calling v1 done**, run against the real environment; they do **not** block starting the build. `broker/` **may start in parallel against Aedes defaults** and is buildable now; what item 1(b)/item 2 gate is the broker's **ws subprotocol/path/auth config being final** (provisional until the fork-parity spike returns; see build-plan §5). Item 3 (adopt-vs-build) is **resolved in favor of building** (§12.6); only an ergonomic/coverage confirmation against the real specs remains.

### Step 2 — Build the v1 core (only if build is confirmed)
**First milestone — M0 walking skeleton (build-plan §3), re-pointed risk-first + floor-first (grilling realignment):** broker (1) + spec registry (2) + L1 (3) + a *minimal* control plane, plus **discovery** (`GET /topics` + `offbook topics`) and an **`mqtt.js`⟷Aedes ws harness**, form the first **dogfoodable** slice. **M0 gate (two parts):** (i) a browser-style `mqtt.js` client connects to the Aedes ws listener, subscribes, and receives a **retained** message — retiring the *doable half* of the #1 hard gate (WS fidelity, §12.1) as the first artifact; and (ii) `offbook topics` lists every topic/shape/direction from the spec — the zero-config **floor** (design §1/§9). **`offbook demo`** (ephemeral mock over a **bundled** spec — no git, so item 6 is *not* on this path — catching a deliberately off-contract publish in `/validation`) ships in M0 too, but as an **output, not the gate**: it is guaranteed-green by construction, so it demonstrates the mechanism rather than certifying the milestone, and validation-as-value stays *provisional* until the §12 gates pass (design §1). Then enrich upward through the rest:
