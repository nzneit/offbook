...
**First milestone — M0 walking skeleton (build-plan §3):** items 1–4 below + a *minimal* control plane (`POST /publish` + `GET /validation` only) + the **`offbook demo`** command (ephemeral mock over a **bundled** demo spec — no git, so item 6 is *not* on this path) form the first **dogfoodable** slice. M0 gate: `offbook demo` catches a deliberately off-contract publish in `/validation` — the **validation wedge** (the v1 lead, design §1/§5) witnessed before scenarios/timing/ingestion land. Then enrich upward through the rest:
