...
- **Contents:** representative specs covering **both directions** (toClient / fromClient) and the known-hard cases — external `$ref`, `$id`, `allOf` / `oneOf` / `anyOf` — so L1 and validation can be developed and the §5 correctness bar actually tested.
- **Refs:** §5, §4, §12.4.
