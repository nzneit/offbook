...
- **Contents:**
  - **TypeScript interfaces** for: the normalized message model `{ topic, direction, payload, qos?, retain?, delay? }`; the broker module (`onInbound` / `emit` / `setRetained` / lifecycle); `Resolver (service, version) → spec`; `VersionSource (environment) → { service: version }`; behavior-layer registration (L3 / L2 / L1); the validation-result shape.
  - **Control-plane HTTP API contract** (OpenAPI form): real method / path / request-schema / response-schema / status / error-format for the §9 endpoints (`GET /topics`, `/state`, `POST /publish`, `/trigger/{scenario}`, `/reset`, `/mode`, `GET /validation`, `/specs`). This is the substrate the CLI just wraps.
...
- [ ] **Deliverable:** new doc `offbook-build-plan.md`
...
- [ ] **Deliverable:** `fixtures/asyncapi/` (committed sample specs)
- **Why it blocks:** the tool consumes AsyncAPI specs, but agents can't see the real proprietary ones.
- **Contents:** representative specs covering **both directions** (toSpa / fromSpa) and the known-hard cases — external `$ref`, `$id`, `allOf` / `oneOf` / `anyOf` — so L1 and validation can be developed and the §5 correctness bar actually tested.
- **Refs:** §5, §4, §12.4.

### P4 — Specify the two prerequisite spikes as deliverables
- [ ] **Deliverable:** short section in `offbook-build-plan.md` (or `offbook-spikes.md`)
...
3. **P2 + P3** — build plan + fixtures. ← **next**
