...
| F20 | Recommended resolution adopted as-is: adoption.md §10 "Staleness honesty" and R-043 in REQUIREMENTS.md rewording the `specs update` skip condition from "reached via bare `--ctrl-port` with no run dir" to "whenever `--ctrl-port` is passed (the target server's run dir correspondence is unverified)" with rationale. Both sources now state the same skip condition exactly (§10, R-043, and code all agree). Verified `bun scripts/check-docs.ts` exit 0. | `docs/specs/adoption.md`, `REQUIREMENTS.md` | subagent batch F | 2026-08-10 |
