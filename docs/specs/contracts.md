...
- **`up` / `down` are NOT HTTP endpoints — they are process management (G14).** No request can start the server that would serve it; `offbook up` **spawns the server detached, writes a PID + port runfile (`<runDir>/offbook.run`: pid + the three ports + `startedAt` + the per-launch `token` + `host` — D-032; `<runDir>` = `config.runDir`, default `.offbook/` under the project directory (`up [dir]`), else cwd; `--run-dir` stays cwd-relative — §1a, D-032), and probes the control-plane port for readiness**; it also redirects the detached server's stdout/stderr to `<runDir>/offbook.log` (alongside the runfile — **text**, size-capped + one rotation, **appended** across `up --watch` restarts so it stays continuous; it carries the design §8 credential-attribution and l2 §8 hot-reload notices, and `offbook logs`/`status` read it — EO1). `offbook down` reads the runfile and signals the process, leaving the log for post-mortem. (`offbook specs update` *is* HTTP — it hits `POST /v1/specs/refresh` above so the running registry doesn't go stale.)
<!-- anchor: R-046 -->
