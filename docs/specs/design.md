...
**Mental model:** *MSW-for-MQTT, with live **bidirectional** contract validation* — and unlike **Pact**'s CI-time consumer-driven handshake, it's a live local mock the browser app talks to continuously. **Validation is the *lead* — the value the tool leads with and demonstrates first (§5).** But it is a *provisional* lead: it is fully shippable and demonstrable on the very *first run* (the developer can **witness a break get caught** — see `offbook demo`, §9), yet its real-world value stays **unproven until the two hard gates pass** (§12: WS-transport fidelity + a real-spec-corpus audit) — a mock that validates *green* against a vacuous or stale spec is false confidence (§7). So the design leads with validation for its demonstrability while treating its *delivered* value as tenuous-until-proven. Beneath it sit two **floors** — values robust to whatever the gates reveal: **discovery** (the always-current topic/shape catalog; zero-config; holds even against weak specs — see *Discoverability*, §9) and **regression-replication** (reproduce a prod/lower-env bug locally as a deterministic test — §6 seeded repro, §9 moment 3). **Async/timing fidelity (§6)** is *supporting*: its differentiated form (adversarial faults) is v2, so v1 ships `delay` only. Naming a single lead keeps the pitch a sentence; naming the floors keeps it honest about what survives if the lead's assumptions don't.
...
- **Not** emitting TypeScript types or other artifacts for external consumers. Any schemas generated are internal to the mocking/faking stack only. (We are deliberately *not* solving the browser application's copy-paste habit by emitting types; we build around the browser application as-is.) **Ceiling this imposes (stated honestly, since discovery and rot-elimination are v1 *floors* — §1):** discovery delivers *always-current documentation + a machine-readable catalog* (`GET /topics`, `offbook topics --json`), **not** a source-of-truth wired into app code; rot-elimination means *the mock and catalog never rot*, **not** that the app's copy-pasted constants never rot. An adopter who wants to close that loop can codegen from `/topics` themselves — mechanism exposed, policy left to them — and an optional codegen sidecar consuming that same endpoint is a v2-if-ever conversation, additive and outside the core, **not** a reversal of this non-goal.
...
- **All MQTT/Aedes interaction lives behind one thin internal broker module** (`onInbound`, `onSubscribe`, `emit`, `getState`, lifecycle) — the de-facto transport adapter; Aedes is one implementation. See `contracts.md` §2.
...
This is the primary purpose, directly targeting "drift undetected until deploy." — and the **lead** (§1): the job the tool leads with, and the one `offbook demo` shows on run #1 (a *provisional* lead — its delivered value stays tenuous until the §12 gates pass).
...
3. **Time-to-first-value** — one command, seconds: `offbook demo` (§9) is the measurable on-ramp to a witnessed catch. *(Caveat: `demo` runs a **bundled** spec with a **deliberately planted** break, so it is guaranteed green by construction — it measures that the *mechanism* works, not that the tool *delivers value* against your real corpus. That is what the §12 hard gates prove; until they pass, treat this metric as a mechanism check, not a value proof.)*
...
**One candidate to pull forward (gated on evidence, grilling realignment).** The v1 `delay`-only scope rests on an *unverified* assumption: that the async bugs this project has hit are "the mock answered instantly" bugs (which `delay` surfaces) rather than ordering/duplicate/race bugs (which it cannot). That taxonomy is currently a **hopeful estimate** — record-keeping is immature — so verifying it against real bug history is real-environment homework, not a build-blocker. *If* that review shows ordering/duplicate bugs dominate, pull exactly **one** primitive forward into v1: **seeded QoS1 duplicate delivery** — cheapest to build, most MQTT-authentic ("at-least-once genuinely means twice"), and the primitive most often needed to *reproduce* a real regression, since the v1 delay-only limit otherwise caps regression-replication (§9 moment 3) to non-ordering bugs.
...
- **(a · §7a) Initial state on connect — always on (with a per-channel opt-out, below).** The engine guarantees retained state exists for any **materialized** instance, so the UI renders populated immediately (no blank UI until the next tick). **Whether** that state pre-exists or is created on demand splits by whether the `toClient` channel address is parametrized — **eager** for non-parametrized channels, **lazy** (created at first concrete subscribe/command) for parametrized ones. L1/L3 publish initial state with `retain: true`; L1's initial-state faking is **per-instance** — keyed by the instance's params (`fake(channel, params)`), so a multi-device `seedInstances` set renders **distinct** devices, not N identical ones (contracts §3, CR1). This is the materialization policy, owned by the engine-side `InstanceRegistry` (the **normative rules + types are in `contracts.md` §2** — this elaborates them with examples + rationale):
  ...
  - **Wildcard subscribe** (`SUBSCRIBE state/+` or `state/#`): emit the **existing retained state for every topic matching the filter** — via Aedes' **native retained delivery** from its own retained store (filter tested by `matchesFilter`, F6; R3 — the subscribe hot-path does **not** call `getState()`), **not** a parallel materialized-instance ledger — and **never invent** a param value. A wildcard carries no binding to de-wildcard, so it can only replay what already exists (a cleared topic is excluded; an off-ledger L3/L2 retained publish is included). *(MQTT `+`/`#` are subscribe-side filters here, distinct from the channel-address `{param}` the registry matcher resolves — `contracts.md` §1.)*
  - **Optional `seedInstances`** — a `ServiceConfig` field (`contracts.md` §6) keyed by channel address, whose value is a **list of param-maps** so multi-param channels work (e.g. `state/{deviceId}: [{ deviceId: thermostat-1 }, { deviceId: thermostat-2 }]`) — pre-materializes a deterministic demo set at startup, so onboarding isn't a blank UI even before any subscribe or command.
...
The result of resolution is always written down. The lockfile is both the **reproducibility guarantee** and the **debug surface for the resolution layer itself**. The per-service entry below is a **v2-rich illustration** — the **canonical v1 on-disk shape is `contracts.md` §6** (kebab-case, `resolution-strategy: branch`, the **full** `resolved-sha`, and no `resolved-version`); the extra fields shown here (`resolution-strategy: git-tag`, `resolved-version`) are **v2-only**:
...
- **Resolver interface:** `resolve(repo, ref, specPath) → Promise<ResolvedSpec>`, implemented as `GitRefResolver` in **both** v1 and v2 (fetching a spec at a git ref is identical regardless of ref kind). The v1↔v2 difference is *ref selection*: v1 uses the per-service `branch` (default `main`); v2 resolves a requested semver → a pinned tag/sha. See `contracts.md` §6.
...
- `offbook init [--dir .]` — scaffold a fresh project (the onboarding cold-start, EI1; templates respecified as fenced reference documents by R-041/D-028): writes, **only if absent** (re-run refuses, nonzero), `services.yaml` (`gitHost` as a **commented example, never an active placeholder** — unset must stay the true config state so a slug hits the clean G20 error, the contracts §6 EI1 amendment — plus one fenced worked example), `environments.yaml` (purpose statement + a fenced worked example), `scenarios/00-example.yaml` (the l2 §0 sample), a **project `README.md`** (install steps with the observed-origin clone URL, adoption §8), an **empty `handlers/`** dir (so `up --watch` has a target), and a **`.gitignore`** that ignores the run-artifact dir (`.offbook/` — the runfile + `offbook.log`; `specs.lock` stays **tracked**); prints next steps (`set gitHost + your services in services.yaml, validate with offbook doctor as you edit, then offbook up`). Does **not** scaffold `specs.lock` (generated by `up`).
- `offbook demo` — **zero-setup first value (validation — the *lead* — witnessed on run #1, §1/§5).** Boots an *ephemeral* mock against a **bundled** demo spec (a fixture promoted to a shipped demo asset — no `services.yaml`, no git, no `runDir` writes), seeds populated state, then publishes a deliberately **off-contract** payload and shows it caught in `/validation` within seconds, with narration (*"↑ Offbook just caught a contract break that would have shipped silently"*). Ephemeral and self-tearing-down — distinct from `init`, which scaffolds a real project to adopt. This is the evaluation on-ramp that precedes moment 1 (and the only zero-git way to see the tool run, now that spec-load is fatal — §7).
- `offbook up [--watch] [--ci] [--strict] [--seed N] [--ws-port N] [--tcp-port N] [--ctrl-port N]` / `offbook down` — lifecycle. **Two boot profiles over `DEFAULT_CONFIG`:** the **interactive default** (`wallClock=true` for human-perceptible timing, `mode=autonomous`, `strict=false` so a bad scenario surfaces in `/diagnostics` instead of crashing the dev's server), and **`--ci`** — the moment-4 one-flag CI boot that co-sets `mode=passive`, `wallClock=false` (fast-virtual; CI pays no real wall time), and `strict=true`, and forces `--watch` off; the determinism gate's *boots-passive* requirement (§5/contracts §3) **is** `--ci`, which then asserts `GET /mode==passive`. `--strict` exposes the fatal-scenario-load behavior on its own (e.g. while authoring) without the rest of the CI profile. *(`--frozen` — pinned spec resolution — is **v2**; v1 always fetches the branch tip, §7/§11.)* `--watch` (autonomous-only) restarts the server on `handlers/**/*.ts` changes so L3 code edits take effect without a manual `down && up` (EH1; off under `--ci`/`passive`). On a **fresh project** (empty `scenarios/`+`handlers/`), `up` prints one honest orientation line — *"N topics served from L1 schema-fakes (valid filler); author behavior in scenarios/\*.yaml (L2) or handlers/\*.ts (L3); discover: offbook topics"* — suppressed once ≥1 scenario/handler loads (EI2). **Lifecycle is defensive** (`contracts.md` §5, P7): `up` preflights the three ports (foreground error on conflict, not a detached `EADDRINUSE`), refuses to double-start a live instance, and auto-reclaims a stale runfile; `down` is idempotent. `up`/`status` **print the client connect target** (`ws://localhost:<brokerWsPort>`, MQTT 3.1.1) so the dev knows where to point the browser app, and `--ws-port`/`--tcp-port`/`--ctrl-port` let two environments run side-by-side. **Repoint convention:** have the app read its broker URL from an env var (e.g. `MQTT_URL`) so mock↔real-backend is a one-line switch — and on localhost use plain `ws://`, not `wss://`
...
- `offbook scenarios` — list authored L2 scenarios (`GET /v1/scenarios`, P8): per scenario, **name · reactive (`when` topic) / on-demand · step count · source file** — triggerable behavior made as discoverable as `offbook topics` makes topics.
- `offbook scenario <name> [--param k=v]... [--payload <json> | --payload-file <path> | --payload -] [--wait]` / `offbook reset [--seed N]` — debugging / test control; repeatable `--param k=v` binds the scenario's `{{param}}` captures and `--payload*` supplies the inbound payload (EQ4). `reset --seed N` re-seeds (wraps `POST /reset {seed?}`); the active **seed is the shareable repro token** — echoed by `reset`/`status`, set at boot via `up --seed N`. *(The verb stays `scenario`, not `trigger` — CLI verbs are deliberately decoupled from endpoint names; this hits `POST /v1/trigger/{name}`.)*
...
- `offbook check [--since <seq>]` — **the one-command CI gate (P8):** exits **nonzero iff the client sent anything off-contract** (`summary.byOrigin.client > 0`) since the last `reset` baseline (or `--since <seq>`), printing the distinct-break count. Reads `GET /v1/validation`; `offbook check || exit 1` in a test teardown replaces the raw `sinceSeq`+`jq` assertion.
...
- `offbook status` — one-shot overview: running?/pid/ports, mode + seed, the **client connect target** (`ws://localhost:<wsPort>`, P7), each service's `resolutionMode` + short-SHA + channel count + **spec age** (P2 Mode 3), the `/validation` summary + **breaks-caught scoreboard** (P6), and the **`/diagnostics` error/warn counts** (P8 — so a silently-failed scenario/spec doesn't read as "0 errors"); composes the runfile + `/mode` + `/specs` + `/validation` + `/diagnostics`; `not running` + nonzero exit when the control plane is down (EO4)
...
Indicative endpoints (substrate the CLI wraps): `GET /topics`, `GET /state`, `POST /publish`, `POST /trigger/{name}`, `POST /reset`, `POST /mode`, `GET /mode`, `GET /validation`, `GET /specs`, `GET /diagnostics`, `GET /scenarios`. **Now pinned — request/response shapes, the `/v1` prefix, and conventions — in `contracts.md` §5.** A `resolve` dry-run surface lands in v2.

---

## 10. L2 Scenario Authoring Process — RESOLVED (see `l2-scenarios.md`)

> **Resolved (P0).** This was the most load-bearing open thread — L2 is the layer developers actually write, and if authoring is tedious the tool never gets enriched past the L1 floor. It is now **decided in `l2-scenarios.md`** (status: *Decided*), which is **canonical for the authoring format** and unblocks handoff Step 2.8. The questions below are kept as an *answered* checklist; the authoritative detail lives in that doc.

Questions — all answered in `l2-scenarios.md`:
...
> **Two of these are *hard gates* on "v1 done," not mere action items** (grilling realignment, 2026-06-30): the **WS-transport fidelity spike** (item 1) and a **real-spec-corpus audit** (item 1a). Both are empirical and largely **air-gapped from the build environment** — they run against the real browser app and the real specs. A green `offbook demo` proves the *mechanism*; only these two gates prove the tool *delivers value*. Treat them as blocking acceptance, not parallel niceties.

1. **Verify the transport/WS-fidelity spike (highest-priority de-risk; hard gate).** Point the real browser application's MQTT client at a bare Aedes ws listener and confirm it connects, subscribes, and receives a retained message **using the same protocol level (MQTT 3.1.1), WS path, subprotocol, and auth shape it uses against the prod fork**. This is the probe for the one suspected divergence (the fork's WebSocket transport, §3). **Split the gate into two halves so the doable half isn't held hostage by the air-gapped one:** (a) *doable in any environment* — an `mqtt.js` client (a close-enough stand-in for the SPA's client) connects to a bare Aedes ws listener, subscribes, and receives a **retained** message; build this as the **M0 gate** (build-plan §3). (b) *air-gapped homework* — confirm the **proprietary-fork** WS handshake (subprotocol/path/headers/TLS/auth) matches, using the real app's captured `connect()` (item 2). Half (a) proves Aedes speaks browser-MQTT-over-ws at all; only (b) proves parity with prod. If the app connects unchanged, the fork's WS behavior is close enough; if not, you've found the divergence cheaply and can bend Aedes's listener/hooks to match.
1a. **Real-spec-corpus audit (hard gate; air-gapped homework).** Before trusting v1 against real work, pull the actual AsyncAPI specs for every service the browser app consumes and grade each channel against §7's three modes (loads? vacuous? fresh?) — **and** extract the org's actual MQTT-binding/transport conventions. The latter matters because every transport/binding test fixture is **hand-authored** (no public MQTT-over-WebSockets AsyncAPI idiom exists in the wild), so a hand-authored fixture silently encodes assumptions about the real specs that only this audit can confirm — otherwise the fixtures pass green while the real specs fail, recreating the air-gap one level down. If the corpus is strong, validation's value is real; if weak, the first unit of value is upstream spec hygiene and validation stays tenuous (§1).
2. **Capture the browser application's actual `connect()` call** (auth fields, ws URL/path, subprotocol, exact protocol level — confirm 3.1.1 vs the older 3.1) — resolves auth design (§8) and feeds the spike above. Also note whether the browser application/services use **QoS 2** anywhere (the contract allows 0/1/2 per `contracts.md` §1; QoS 2 is latent capacity until confirmed), and capture each topic's **QoS/retain** so the spec-binding-vs-observed divergence warn-log (P1.D4 tier-3) has a baseline.
...
5. **L2 authoring format** (§10) — **Resolved** in `l2-scenarios.md` (the dedicated pass is done); it was the thread most determining whether v1 *feels* right.
6. **Stage-0 adopt-vs-build check.** **Resolved — verified via external research + Microcks/Specmatic source-code reading (2026-06): the build is justified, scoped to the gap below.** Neither tool covers the combination off-the-shelf:
   ...
   - **AsyncAPI's own toolchain** (the most reflexive reach for a shop already publishing AsyncAPI): **Glee** is an event-application *framework* for *building* services (it runs your handlers), not a spec-derived mock that loudly validates what a *client* publishes; the **Generator**/**Modelina** emit code/docs/types from a spec — not a live broker a browser `mqtt.js` client connects to.
   - **Unserved core (confirmed not covered by any of these):** *embedded in-process broker · MQTT-over-WS for a browser application · loud continuous browser-app→service validation · stateful templated request/response scenarios · MQTT-semantic timing/fault injection.* This is exactly Offbook's scope, so building is justified.
   - **Reuse rather than rebuild:** AsyncAPI schema validation (already adopted via `@asyncapi/parser` + Ajv, §5) and the provider-conformance pattern (maps onto the "CI asserts on the `/validation` log" idea, §5). Note too that Microcks' own MQTT path depends on an *external* standard broker's WS listener for browser reach — precisely the role Aedes plays here (§3), so even the off-the-shelf tool implicitly needs the component we're building. **Build-vs-compose:** the honest n=1 alternative isn't only "adopt a competitor" but "glue the libs we already depend on (Aedes + `json-schema-faker` + Ajv + a thin Hono plane) + scripts" — yet the differentiated parts (the seeded virtual-clock scheduler, loud bidirectional surfacing, content-hashed resolution) aren't off-the-shelf glue, which is what makes the bespoke engine the irreducible core, not gold-plating.
