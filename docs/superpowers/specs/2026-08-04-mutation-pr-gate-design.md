...
- Unit tests green under full `bun test` (per-file coverage floors judge the `.mjs`; gate on exit code, not printed counts). Required fixtures: a rename record (`R100`, three-field), a deleted-test-file diff that must pull the sibling source, and a report JSON exercising every status in the enum including the zero-valid case.
