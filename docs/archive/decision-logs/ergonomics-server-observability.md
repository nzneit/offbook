...
**Status:** **Resolved** (2026-06-30) — all 4 decided in dialog as one *observe-the-detached-server* unit: **text** log at `<runDir>/offbook.log` (EO1), `offbook logs [-f]` (EO2), **client-side** `--watch` on `validation`/`diagnostics` (EO3), and `offbook status` (EO4). Folded into contracts §5 (the G14 runfile note), design §9 (CLI surface), and build-plan Tier 4. See the **Decision log**.
...
- **Status:** ☑ **resolved** (2026-06-30) — text log at **`<runDir>/offbook.log`** (alongside the runfile), size-capped + one rotation, path printed by `up`, **appended across `up --watch` restarts**, left on `down` for post-mortem. Folded → contracts §5 (G14 runfile note), design §9.
...
- **Depends on:** EO1.
- **Status:** ☑ **resolved** (2026-06-30) — `offbook logs [-f] [-n N]` reads/tails `<runDir>/offbook.log` resolved from the runfile. Folded → design §9, build-plan Tier 4.
...
- **Status:** ☑ **resolved** (2026-06-30) — **client-side `--watch`** (poll `?sinceSeq=<last>` every `--interval`, render new entries via ER2's reusable function) over a server-side SSE stream — no contract/streaming surface added, consistent with EC1 keeping the API poll-based. Adds a base `offbook diagnostics` command too. Folded → design §9, build-plan Tier 4.
...
- **Status:** ☑ **resolved** (2026-06-30) — `offbook status` composes the runfile + `/mode` + `/specs` + `/validation` summary into the one-shot overview; `not running` + nonzero exit when the control plane is down. Folded → design §9, build-plan Tier 4.
...
|---|---|---|---|---|
| EO1 | Detached server logs to a **text** file at `<runDir>/offbook.log` (alongside the runfile), size-capped + one rotation, path printed by `up`, appended across `up --watch` restarts, left on `down` | offbook-contracts.md §5 (G14 note); offbook-design.md §9 | CodeReviewJoe | 2026-06-30 |
| EO2 | `offbook logs [-f] [-n N]` reads/tails `<runDir>/offbook.log` resolved from the runfile | offbook-design.md §9; offbook-build-plan.md Tier 4 | CodeReviewJoe | 2026-06-30 |
| EO3 | **Client-side** `--watch [--interval]` on `validation`/`diagnostics` (poll `?sinceSeq=`, render via ER2's function) over server-side SSE — no new API surface; adds a base `offbook diagnostics` command | offbook-design.md §9; offbook-build-plan.md Tier 4 | CodeReviewJoe | 2026-06-30 |
| EO4 | `offbook status` composes runfile + `/mode` + `/specs` + `/validation` summary into a one-shot overview; `not running` + nonzero exit when down | offbook-design.md §9; offbook-build-plan.md Tier 4 | CodeReviewJoe | 2026-06-30 |
