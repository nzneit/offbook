...
> **⚠ Superseded by P5.2 (round 4 — `offbook-build-gaps-4.md`):** the lockfile **reader** (`up --frozen`), the atomic `ls-remote`→by-SHA fetch, and the F17 fallback are now **v2**. v1 ships the **writer** only — it shallow-fetches the branch tip and records the post-fetch `resolved-sha` but never reads it back, so the byte-identical `up --frozen` acceptance below is a **v2** check. Canonical: contracts §6/§7 + design §11.
...
| G4 | Lockfile **reader** wired (`up --frozen` re-resolves by `resolvedSha`); atomic `ls-remote` → fetch-that-SHA (`git fetch <sha>`, not `git archive --remote`) **— [superseded by P5.2 (round 4): reader + atomic by-SHA + F17 → v2; v1 = writer only]** | contracts §6; build-plan |
