...
| F12 | Wording-only fix (recommended resolution adopted as-is): the attribution message now claims only the verified control port ("another offbook owns the control port `<n>`"), lists other busy labels only as an "also busy" clause when present, drops "likely the demo", and points at `offbook down` "in that project's directory"; applied identically at both call sites (`preflightPorts` in `index.ts`, the `ports` doctor check). Pinned strings updated in both test files, plus the two verbatim quotes in `docs/specs/adoption.md` §10 and `skills/offbook-onboard/SKILL.md`'s refusals section (the SKILL.md quote otherwise would have gone stale-but-gate-green). | `src/cli/index.ts`, `src/cli/doctor.ts`, `src/cli/doctor.test.ts`, `test/cli-dispatch.test.ts`, `docs/specs/adoption.md`, `skills/offbook-onboard/SKILL.md` | subagent batch B | 2026-08-09 |
| F5 | Both parts of the recommended resolution adopted as-is: (a) new `src/cli/runfile.test.ts` unit-tests `probeOffbook` directly against real listeners (non-mode-JSON responder, never-responds raw-socket listener bounded by a custom `timeoutMs`, `{"mode":"passive"}`/`{"mode":"autonomous"}` responders, and a no-listener refused-connection case); (b) one doctor `ports` case and one `up` preflight case each put a plain `Bun.serve` (no `mode` field) on the *ctrl* port and assert the generic `port(s) busy`/`port(s) in use` message with no "another offbook owns"/"another directory" text. By-hand spot-check: stubbed `probeOffbook` to `return true`, confirmed all 3 new false-expecting `runfile.test.ts` cases plus both new doctor/cli-dispatch negative cases failed (filtered `-t` runs, isolated from unrelated pre-existing tests also perturbed by the global stub), then reverted (`git diff` on `runfile.ts` clean before restoring). | `src/cli/runfile.test.ts` (new), `src/cli/doctor.test.ts`, `test/cli-dispatch.test.ts` | subagent batch B | 2026-08-09 |
| F4 | Mechanical resolution adopted as-is: `clientsFromLog` now sanitizes `fields.clientId` with `.replace(/\p{Cc}/gu, "?")` immediately after the string-type guard, so every consumer (status text, `status --json`, any future one) gets a clean string. New test builds the ESC byte via `String.fromCharCode(27)` (not a source literal), JSON-encodes it into a fabricated `ws-connect` log line, and asserts the recovered `last.clientId` contains neither the raw ESC nor any character with code < 32 or 127. | `src/cli/index.ts`, `test/cli-dispatch.test.ts` | subagent batch C | 2026-08-09 |
| F8 | Simple variant adopted (not the fuller `bootProject`-replumb): `services.yaml` is read once before `bootProject`/`bootDemo` is called (guarded by the same `existsSync` check `bootProject` itself uses, so a genuinely-missing file still surfaces `bootProject`'s friendly `offbook init` fatal error rather than a raw read failure), and the boot line is now logged with no `await` between `composed.start()` resolving and the `log(...)` call. The hash input is that pre-boot read, never a second post-start read. Boot-line format unchanged (`boot: services.yaml sha256:<64-hex>` / `boot: bundled demo spec`); existing boot-line/staleness/clients tests pass unmodified. | `src/cli/serve.ts` | subagent batch C | 2026-08-09 |
| F7 | Mechanical resolution adopted as-is: after `demo-serve.test.ts`'s existing real ws connect (`demo-serve-probe`), the test now runs `status --run-dir` and `status --run-dir --json` against the live spawned server and asserts the nonzero text line (`clients: 1 connect(s) this run · last demo-serve-probe at `) and the JSON `clients` object (`connects: 1`, `last.clientId` a string). Spot-check: temporarily replaced the nonzero template's `${clients.connects}`/`${clients.last?.clientId}` with `${0}`/`${clients.last?.at}` in `src/cli/index.ts`, reran the file — the new assertion failed on the corrupted text (`clients: 0 connect(s) this run · last <timestamp> at <timestamp>` vs expected `clients: 1 ... last demo-serve-probe at `); reverted, file matches HEAD (`git diff` clean), reran — 2 pass, 0 fail. | `test/demo-serve.test.ts` | subagent batch C | 2026-08-09 |
| F16 | Mechanical resolution adopted as-is: added `expect(r2.checks[0].status).toBe("pass");` immediately after the existing `detail` assertion in the not-installed case, so a regression to `fail` (which would break every adopter repo that hasn't opted into the skill) now fails this test instead of passing silently. | `src/cli/doctor.test.ts` | subagent batch C | 2026-08-09 |
| F14 | Recommended resolution adopted as-is: `checkoutCommit`/`checkoutOrigin` now share a private `isRepoRoot(root)` guard — `git rev-parse --show-toplevel` from `root` must realpath-equal `root` itself (both sides run through `realpathSync`) before any git state is trusted; otherwise `checkoutCommit` returns `"unknown"` and `checkoutOrigin` returns `undefined`. This closes the walk-up-to-an-enclosing-repo path for both `--version` (`index.ts` calls `checkoutCommit(root)` with the same `repoRoot()`) and the skill install stamp (`skill.ts` calls both with no args, same default). New test builds an outer repo with an origin remote, creates an offbook-shaped subdirectory with no `.git` of its own inside it, and asserts `checkoutCommit` reports `"unknown"` and `checkoutOrigin` reports `undefined` from that subdirectory — proving the guard fires even though the *outer* repo has a real sha and origin to offer. | `src/cli/checkout.ts`, `src/cli/checkout.test.ts` | subagent batch D | 2026-08-09 |
| F11 | Recommended resolution adopted as-is (keep the field, relativize): a new private `relativizeHome(path)` helper in `skill.ts` (no `escapeRegExp` — `escapeRegExp` doesn't exist in this codebase; used a `startsWith(homedir() + sep)` + `slice` check instead, plus an exact-match case for `path === home`) replaces an exact homedir prefix with `~`; `copySkill`'s stamp now writes `sourcePath: relativizeHome(repoRoot())` with a best-effort/installing-machine-only comment. `src/cli/skill.test.ts`'s fresh-install stamp test gained two assertions (no `/home/` substring; starts with `~`, since this checkout lives under `$HOME` in every environment the gate runs in). `docs/specs/adoption.md` §9's "Provenance stamp" bullet (the sentence enumerating the stamp's fields) now names the relativization and its rationale. | `src/cli/skill.ts`, `src/cli/skill.test.ts`, `docs/specs/adoption.md` | subagent batch D | 2026-08-09 |
| F13 | Mechanical resolution adopted as-is: the `--dest` below-toplevel check now compares `realpathSync(top)` against `realpathSync(targetRoot)` instead of `resolve(top) !== targetRoot`, so a repo reached through a symlinked `--dest` no longer spuriously warns (git's `--show-toplevel` returns the physical path; the old code compared it against the still-logical `targetRoot`). New test creates a real repo, a symlink pointing at it in a separate temp dir, installs via `--dest <symlink>` and asserts no "below the repo toplevel" text, then installs into a genuine subdirectory and asserts the warning still fires. Spot-checked by stashing the fix and re-running the new test alone: it failed (1 fail) against the pre-fix `resolve()` comparison; restored, 0 failures. | `src/cli/skill.ts`, `src/cli/skill.test.ts` | subagent batch D | 2026-08-09 |
| F9 | Warn-and-proceed adopted as controller-decided (recommended): a `.claude` (or any ancestor of the install path) that is itself a symlink is no longer refused or silently mis-advised. Two new helpers in `skill.ts` — `deepestExistingAncestor`/`realpathThroughExistingAncestors` walk up from `destDir` to the closest path that actually exists, `realpathSync` that (following any symlinks), and re-attach the not-yet-created remainder literally, giving the true on-disk landing spot even before `copySkill` creates it; `isWithinRoot` does the containment check against `targetRoot`'s own (same-method) resolved path. When the resolved `destDir` escapes the resolved `targetRoot`, `cmdSkill` prints `⚠ .claude here resolves outside this repo (<resolved path>) — the skill will install there and will NOT propagate via this repo's commits` before the exists/compare block (composes with F2's non-directory guard and F13's toplevel check, which both run in the same function), and the closing success line drops "— commit it so teammates get the skill" instead of pairing it with a warning that contradicts it. New test symlinks a repo's `.claude` to a sibling temp dir, does a first install (asserts the warning, the corrected success line, and the file landing under the sibling's resolved path), edits the installed `SKILL.md` to create drift, then `--force` reinstalls (asserts the warning again, the file clean-replaced back, and an unrelated marker file placed directly in the sibling dir — outside the `skills/offbook-onboard` leaf — survives, proving `rmSync` stayed bounded). Spot-checked by stashing only the `skill.ts` fix and re-running the new test alone: it failed (1 fail) against pre-fix `cmdSkill`; restored, 0 failures. | `src/cli/skill.ts`, `src/cli/skill.test.ts` | subagent batch D | 2026-08-09 |
| F19 | Mechanical resolution adopted as-is: new case installs via `--dest <plain mkdtemp dir>` (no `.git` anywhere in its ancestry) and asserts exit 0, the existing "not inside a git repo — the skill cannot propagate" warning text, and that `SKILL.md` actually landed under `<plain>/.claude/skills/offbook-onboard/`, distinguishing this warn-and-proceed branch from the no-`--dest` path's refuse-with-throw (already covered by the "no positional: outside a git repo errors" test). Spot-checked by temporarily changing the branch's `io.err(...)` to `throw new CliError(...)`: the new test failed (1 fail); reverted, `git diff` on `skill.ts` clean, 0 failures. | `src/cli/skill.test.ts` | subagent batch D | 2026-08-09 |
| F10 | Mechanical wording fix adopted as-is: the "Refusals you may hit" bullet no longer pre-authorizes `--force` on drift. It now reads: run `offbook skill install` (no `--force`) and show the human the drift listing it prints; local edits are drift — upstream anything worth keeping, then re-run with `offbook skill install --force` only once the human approves the clean-replace — matching step 4's existing human-approval discipline. Verified `grep -n "install --force" skills/offbook-onboard/SKILL.md` returns exactly one line, inside the approve-first sentence; both verb forms (`skill install`, `skill install --force`) remain valid `VERB_FORMS` members so `checkSkillVerbs` still passes; `bun scripts/check-docs.ts` exit 0. No test pins the SKILL.md prose itself (checked `scripts/check-docs.test.ts` and `doctor.test.ts`'s `--force` assertion, which pins `doctor`'s own hint text, not this file), so no other test needed updating. | `skills/offbook-onboard/SKILL.md` | subagent batch D | 2026-08-09 |
| F18 | Both parts of the recommended resolution adopted, mechanism chosen for the shape check (controller decision, lighter option): `extractExample` became `extractExamples`, looping over EVERY `# --- example ---`/`# --- end example ---` marker pair in document order (not just the first via `indexOf`) and asserting the fence count is balanced; a new unit test pins that a template with a second fence has both examples collected, and that an unbalanced fence throws. For the scenario shape check, reused doctor's OWN already-exported checker (`DOCTOR_CHECKS.find(c => c.name === "scenarios")` + the already-exported `runDoctor`) instead of hand-rolling the same shape assertions locally or exporting anything new from `doctor.ts` — lighter than scaffolding a full project and running all 8 `runDoctor` checks (which would also exercise network/ports/deps), since the scenarios check only reads `ctx.projectDir`; each extracted example is written to its own file in a temp dir and checked in one `runDoctor(ctx, [scenariosCheck])` call. Spot-check: planted a second, invalid fence (`services: not-a-map-this-is-invalid`) into the real `INIT_SERVICES` template — the services.yaml test failed (`services.yaml: missing required 'services' map`, 5 pass/1 fail), proving the old first-fence-only extraction would have silently ignored it; reverted (`git diff` clean), reran — 6 pass, 0 fail. | `test/init-templates.test.ts` | subagent batch E | 2026-08-09 |
| F17 | The exported-map variant adopted as recommended: each owning module now exports the real subcommand set it dispatches — `src/cli/skill.ts` exports `SKILL_SUBCOMMANDS = ["install"]` beside `cmdSkill`; `src/cli/index.ts` exports `SPECS_SUBCOMMANDS = ["update"]` beside `cmdSpecs`. `src/cli/verbs.ts` stays a leaf (imports neither). `test/verb-forms.test.ts` adds a new assertion beside the existing first-token comparison: every two-token `VERB_FORMS` entry must appear in its owning module's exported subcommand set (`OWNING_SUBCOMMANDS` maps first token → export). Spot-check: added a fake `"specs prune"` to `VERB_FORMS` (verbs.ts) and `"specs [update] [prune]"` to `USAGE` (index.ts), reran `test/verb-forms.test.ts` — the new test failed alone (`'specs prune' is not a real dispatch subcommand of 'specs'`, 4 pass/1 fail), proving it catches what the old docs-to-docs pin (VERB_FORMS-vs-USAGE) would have blessed; reverted both files (`git diff` clean), reran — 5 pass, 0 fail. | `src/cli/skill.ts`, `src/cli/index.ts`, `test/verb-forms.test.ts` | subagent batch E | 2026-08-09 |
| F15 | Recommended resolution adopted as-is: two new cases in `checkSkillLinks`'s test — `[me](SKILL.md)` (a real intra-skill file, resolves relative to SKILL.md's own directory) errors `[]`, `[gone](missing.md)` errors exactly 1, message contains "broken link". Spot-check: commented out the `existsSync` clause in `checkSkillLinks` (the resolution check the old tests never exercised), reran `scripts/check-docs.test.ts` — the new fail-case test failed (1 fail, 50 pass), confirming the clause was previously deletable with all tests green; restored, 0 failures (51 pass). | `scripts/check-docs.test.ts` | subagent batch E | 2026-08-09 |
| F3 | Both decision points resolved as recommended: (a) over-match fixed by truncating at a shell metacharacter (`#\|&;`) before tokenizing, so `offbook specs # comment` / `offbook specs \| grep x` no longer misread the comment/pipe tail as a second-token subcommand; (b) under-match: fixed the one live instance (SKILL.md's `` `offbook status` `` span, previously wrapped across the step-6 line break) rather than building a multi-line-span scanner — adoption.md §9's residual sentence now states the wrapped-span boundary explicitly (verb name + `offbook` must share one physical line; a wrapped trailing FLAG, e.g. `` `offbook up\n--ci` ``, is unaffected since the verb name itself isn't split). New gate tests pin: a bare `$ offbook up` line (pass), a dead verb in a bare line (fail), `offbook specs # comment` / `offbook specs \| grep x` (pass), a dead verb hidden in a comment/pipe tail (still fails — truncation doesn't swallow the check), and the wrapped-span residual itself (documented as NOT scanned, both the dead-verb case and the safe wrapped-flag case). Spot-check: planted `offbook bogus-dead-verb` as a bare line appended to SKILL.md, `bun scripts/check-docs.ts` exited 1 naming it; reverted, exit 0. `grep -n 'offbook$' skills/offbook-onboard/SKILL.md` returns nothing. | `scripts/check-docs.ts`, `scripts/check-docs.test.ts`, `skills/offbook-onboard/SKILL.md`, `docs/specs/adoption.md` | subagent batch E | 2026-08-09 |
| F6 | Recommended resolution adopted as-is: adoption.md §10's clients-line example and zero case updated from old format `connects: 3, last: web-abc123 14:02:11` to shipped format `clients: 3 connect(s) this run · last web-abc123 at 14:02:11` and `clients: no connects observed this run — is your app pointed at ws://localhost:<port>?` (two concrete forms showing both nonzero and zero states). Verified `grep -n "connects: 3, last" docs/specs/adoption.md` returns nothing; the shipped strings appear in §10; `bun scripts/check-docs.ts` exit 0. | `docs/specs/adoption.md` | subagent batch F | 2026-08-10 |
| F20 | Recommended resolution adopted as-is: adoption.md §10 reworded (skip condition clarified from "reached via bare `--ctrl-port` with no run dir" to "reached via `--ctrl-port`"); R-043's existing "--ctrl-port-only" phrasing already tracked the code and was left as-is. Verified `bun scripts/check-docs.ts` exit 0. | `docs/specs/adoption.md` | subagent batch F | 2026-08-10 |
| F21 | Recommended resolution adopted as-is: removed "`--version` output shape," from src/cli/skill.test.ts entry in adoption.md §6, added reference to `test/verb-forms.test.ts` where the actual test lives. Verified the edit with `bun scripts/check-docs.ts` exit 0. | `docs/specs/adoption.md` | subagent batch F | 2026-08-10 |
| F22 | Recommended resolution adopted as-is: adoption.md §3 check-8 warn column expanded to enumerate all three warn states now present in `src/cli/doctor.ts` (390-394, 407-408, 417): "bundled skill missing ("incomplete offbook checkout?"), installed copy differs ("stale/edited skill — `offbook skill install --force` refreshes it"), or installed copy unreadable/degenerate ("corrupted install — `--force` replaces it")". Verified `bun scripts/check-docs.ts` exit 0. | `docs/specs/adoption.md` | subagent batch F | 2026-08-10 |
| F23 | Recommended resolution adopted as-is: added `README.md` to R-025's init-artifact list (`init` writes `services.yaml`/`environments.yaml`/`scenarios/00-example.yaml`/empty `handlers/`/`README.md`/`.gitignore`), added `skill install` to R-019's verb enumeration (now lists `init/demo/up/down/topics/publish/state/scenarios/scenario/reset/mode/validation/check/diagnostics/logs/status/specs update/skill install`), and updated build-plan.md tier-4 accept clause to include `skill install` in the verb list. Verified `grep -n "README" REQUIREMENTS.md` and both verb enumerations include `skill`; `bun scripts/check-docs.ts` exit 0. | `REQUIREMENTS.md`, `docs/specs/build-plan.md` | subagent batch F | 2026-08-10 |
| F12-residual | R-043 narrative aligned to shipped port-conflict attribution message: REQUIREMENTS.md R-043 updated old quoted message ("an offbook from another directory owns these ports…") to current shipped message ("another offbook owns the control port `<n>`; also busy: `<other labels>` — `offbook down` in that project's directory frees the control port; check the others separately if they persist") to accurately reflect what `up` preflight and doctor's ports check now emit after F12's fix. Verified `bun scripts/check-docs.ts` exit 0. | `REQUIREMENTS.md` | subagent batch F | 2026-08-10 |
| follow-up noted | resolved: `.claude-as-a-file` degenerate shape (install/--force hit raw ENOTDIR; doctor passes) — F2's genre. Decision: refuse-always in both modes, doctor warns. `cmdSkill` now walks the dest ancestors strictly between the target root and the skill leaf (`.claude`, `.claude/skills` — a new exported `blockingAncestor` helper in `skill.ts`) *before* the existing dest-itself checks; if one exists and isn't a directory, it throws a `CliError` naming the exact path in BOTH `install` and `install --force` — unlike the offbook-owned `offbook-onboard` leaf (F2's clean-replace), `.claude` is Claude Code's own shared config dir, so `--force` does not delete it ("`skill install: <path> exists and is not a directory — move or remove it yourself before installing; --force does not apply: <path> is not offbook's to replace`"). Doctor's `skillCheck` reuses the same helper and returns `{ status: "warn", detail: "<path> exists and is not a directory — \`offbook skill install\` cannot install here (move or remove it yourself; --force does not apply)" }` instead of the previous silent "not installed" pass; all other check-8 states (bundled-missing warn, differs warn, unreadable/degenerate warn, not-installed pass, non-repo pass, identical pass) are unchanged. adoption.md §3's check-8 row and §9's "Pinned verb semantics" bullet quote both shipped strings verbatim. | `src/cli/skill.ts`, `src/cli/doctor.ts`, `src/cli/skill.test.ts`, `src/cli/doctor.test.ts`, `docs/specs/adoption.md` | subagent follow-up | 2026-08-10 |
