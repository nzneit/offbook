...
**Companion to:** `offbook-design.md` (decisions/rationale), `offbook-l2-scenarios.md` (P0 — L2 authoring), `offbook-prework.md` (P1 tracker). **Provenance/rationale for every choice here:** `offbook-contracts-decisions.md` (the P1 dialog log). **Section-ref convention:** a bare `§N` refers to **this** document's sections; cross-document refs are always prefixed — `design §N`, `l2 §N`, `contracts §N`.
...
- **`offbook demo` is a third process-management command (not an endpoint).** It boots an **ephemeral** server against a **bundled** demo spec — **no `services.yaml`, no git, nothing written to `runDir`** — over no new HTTP surface (it drives the existing `POST /v1/publish` + `GET /v1/validation`): it seeds populated state, runs a scripted **off-contract** publish, prints the caught `Violation`, and tears down. It exists to show the validation wedge on run #1 (design §1/§9); it is the M0 milestone's acceptance criterion (build-plan §3).
