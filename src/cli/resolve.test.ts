...
// Port BASES for this file (repo convention: unique per file): 19400-19429
// (19428 unused) and 19300-19317, the second block belonging to the
// mutation-hardening round. Those are allocation bases, not necessarily the
// ports bound at runtime: every one goes through port() from test/ports.ts,
// which maps it into this process's claimed band. Band 0 (a normal local run)
// is the identity map, so the numbers below are what you will see; a
// concurrent run claims a higher band and the same bases land elsewhere.
...
// `ctrl` is already a mapped port (every call site passes port(BASE)); the
// parameter is deliberately NOT named `port`, which would shadow the imported
// mapper for anything later added to this body.
function identityServer(
	...
	const full = {
		...
		startedAt: "t",
		demo: false,
		ports: { brokerWsPort: 1, brokerTcpPort: 2, controlPlanePort: ctrl },
		...identity,
	} as ServerIdentity;
...
test("row 1: live cwd runfile + matching token resolves (source cwd) and adopts a pointer", async () => {
	...
	await writeRunfile(runDir, RUN(port(19400), token), { stateDir: state });
	rmSync(pointerPath(state, runDir), { force: true }); // adopt-on-sight must rewrite it
	const server = identityServer(port(19400), {
		token,
		runDir,
		projectDir: cwd,
	});
...
test("row 2: legacy /v1/mode answer resolves live-unverified locally and adopts", async () => {
	...
	const cwd = scratch("offbook-res-cwd-");
	const runDir = join(cwd, ".offbook");
	await writeRunfile(runDir, RUN(port(19401)), { stateDir: state });
...
test("row 3: live pid, silent port — skipped, runfile kept, nothing resolved", async () => {
	...
	const cwd = scratch("offbook-res-cwd-");
	const runDir = join(cwd, ".offbook");
	await writeRunfile(runDir, RUN(port(19402), "22".repeat(16)), {
		stateDir: state,
	});
	const res = await resolveInstance({ cwd, stateDir: state });
...
test("row 4: wrong token — the port belongs to someone else; skipped, never reclaimed", async () => {
	...
	const cwd = scratch("offbook-res-cwd-");
	const runDir = join(cwd, ".offbook");
	await writeRunfile(runDir, RUN(port(19403), "33".repeat(16)), {
		stateDir: state,
	});
	const server = identityServer(port(19403), {
...
test("row 5: dead pid — reclaimed (runfile + pointer gone), M14 note, falls through", async () => {
	...
	await writeRunfile(
		runDir,
		{ ...RUN(port(19404), "55".repeat(16)), pid: dead.pid ?? 4_193_997 },
		{ stateDir: state },
	);
	const res = await resolveInstance({ cwd, stateDir: state });
	expect(res.resolved).toBeUndefined();
...
test("--run-dir accepts a projectDir whose .offbook holds the runfile; no registry fallback", async () => {
	...
	await writeRunfile(runDir, RUN(port(19405), token), { stateDir: state });
	const server = identityServer(port(19405), {
		token,
		runDir,
		projectDir: proj,
	});
...
test("explicit-path dead pid is reported, not reclaimed", async () => {
	...
	await writeRunfile(
		join(proj, ".offbook"),
		{ ...RUN(port(19406), "88".repeat(16)), pid: dead.pid ?? 4_193_990 },
		{ stateDir: state },
	);
	try {
		const res = await resolveInstance({
...
test("pre-D-032 runfile without a token: the served identity's runDir vouches for it (ours fallback)", async () => {
	...
	const proj = scratch("offbook-res-proj-");
	const runDir = join(proj, ".offbook");
	await writeRunfile(runDir, RUN(port(19407)), { stateDir: state });
	let server = identityServer(port(19407), {
		token: "99".repeat(16),
		runDir,
		projectDir: proj,
	});
	...
	await writeRunfile(runDir, RUN(port(19407)), { stateDir: state });
	server = identityServer(port(19407), {
		token: "99".repeat(16),
		runDir: "/somewhere/else/.offbook",
...
test("row 10 on the explicit path: a foreign-host runfile refuses with the wrote-on-host message, exit 2", async () => {
	...
	await writeRunfile(
		runDir,
		RUN(port(19408), "77".repeat(16), "some-other-machine"),
		{
			stateDir: state,
		},
	);
...
test("rows 6-8: registry scan verifies by token, skips the silent, reaps the dead", async () => {
	...
	await writeRunfile(liveRun, RUN(port(19407), token), { stateDir: state });
	const server = identityServer(port(19407), {
		token,
		runDir: liveRun,
		projectDir: liveProj,
	});
	...
	await writeRunfile(silentRun, RUN(port(19408), "99".repeat(16)), {
		stateDir: state,
	});
	...
	await writeRunfile(
		deadRun,
		{ ...RUN(port(19409), "aa".repeat(16)), pid: dead.pid ?? 4_193_996 },
		{ stateDir: state },
	);
	try {
		const res = await resolveInstance({ cwd, stateDir: state });
...
test("row 9: pointer whose runfile is missing — self-heal from a served identity, else guarded reap", async () => {
	...
	await writeRunfile(healRun, RUN(port(19410), token), { stateDir: state });
	rmSync(join(healRun, "offbook.run")); // de-runfiled, pointer dangles
	const server = identityServer(port(19410), {
		token,
		runDir: healRun,
		projectDir: healProj,
		ports: { brokerWsPort: 1, brokerTcpPort: 2, controlPlanePort: port(19410) },
	});
	try {
		const res = await resolveInstance({
			cwd,
			stateDir: state,
			selfHealProbePort: port(19410),
		});
	...
	await writeRunfile(goneRun, RUN(port(19411), "cc".repeat(16)), {
		stateDir: state,
	});
	...
	const res2 = await resolveInstance({
		cwd,
		stateDir: state,
		selfHealProbePort: port(19412), // silent
	});
...
test("row 9 heals the cwd's OWN dangling pointer", async () => {
	...
	await writeRunfile(runDir, RUN(port(19424), token), { stateDir: state });
	rmSync(join(runDir, "offbook.run")); // de-runfiled while standing in its own project
	const server = identityServer(port(19424), {
		token,
		runDir,
		projectDir: proj,
	});
...
test("tiebreak: ancestor-or-equal beats descendant beats sole-non-demo; prefix siblings never match; ambiguity refuses nothing here (resolver reports)", async () => {
	...
	await writeRunfile(aRun, RUN(port(19413), aTok), { stateDir: state });
	await writeRunfile(bRun, RUN(port(19414), bTok), { stateDir: state });
	const aSrv = identityServer(port(19413), {
		token: aTok,
		runDir: aRun,
		projectDir: aProj,
	});
	const bSrv = identityServer(port(19414), {
		token: bTok,
		runDir: bRun,
		projectDir: bProj,
	});
...
test("tiebreak stage 2: the sole strict descendant wins from a parent cwd", async () => {
	...
	await writeRunfile(descRun, RUN(port(19425), descTok), { stateDir: state });
	await writeRunfile(otherRun, RUN(port(19426), otherTok), { stateDir: state });
	const descSrv = identityServer(port(19425), {
		token: descTok,
		runDir: descRun,
		projectDir: descProj,
	});
	const otherSrv = identityServer(port(19426), {
		token: otherTok,
		runDir: otherRun,
...
test("tiebreak stage 3: the sole non-demo wins over a live demo, with the demo-passed-over note", async () => {
	...
	await writeRunfile(runA, RUN(port(19415), tokA), { stateDir: state });
	await writeRunfile(runD, RUN(port(19416), tokD), { stateDir: state });
	const srvA = identityServer(port(19415), {
		token: tokA,
		runDir: runA,
		projectDir: projA,
	});
	const srvD = identityServer(port(19416), {
		token: tokD,
		runDir: runD,
		projectDir: projD,
		demo: true,
...
test("attributeCtrlPort: names the owner only when the claimed runfile carries the served token", async () => {
	...
	await writeRunfile(runDir, RUN(port(19417), token), { stateDir: state });
	const server = identityServer(port(19417), {
		token,
		runDir,
		projectDir: proj,
	});
	try {
		expect(await attributeCtrlPort(port(19417))).toEqual({
			projectDir: proj,
			runDir,
			demo: false,
		});
	...
	const liar = identityServer(port(19418), {
		token: "f4".repeat(16),
		runDir,
		projectDir: proj,
	});
	try {
		expect(await attributeCtrlPort(port(19418))).toBeUndefined();
	} finally {
		liar.stop();
	}
	expect(await attributeCtrlPort(port(19419))).toBeUndefined(); // silence
	rmSync(proj, { recursive: true, force: true });
	rmSync(state, { recursive: true, force: true });
});
...
test("guarded site #1: a pointer rewritten between the scan read and the unlink survives the reap", async () => {
	...
	const runDir = join(proj, ".offbook");
	await writeRunfile(runDir, RUN(port(19420), "a1".repeat(16)), {
		stateDir: state,
	});
...
test("guarded site #5: the self-heal skips when a runfile reappeared before the write", async () => {
	...
	await writeRunfile(runDir, RUN(port(19421), token), { stateDir: state });
	...
	// overwrite it
	const raced = `${JSON.stringify({ ...RUN(port(19421), token), pid: 424242 })}\n`;
	const identity = {
		pid: process.pid,
		token,
		host: hostname(),
		...
		startedAt: "t",
		demo: false,
		ports: { brokerWsPort: 1, brokerTcpPort: 2, controlPlanePort: port(19421) },
	};
	const server = Bun.serve({
		port: port(19421),
		fetch: (req) => {
...
test("tiebreak: a symlinked cwd matches its real subtree (stage 1 through realpath, against a live competitor)", async () => {
	...
	await writeRunfile(runDir, RUN(port(19422), token), { stateDir: state });
	await writeRunfile(otherRun, RUN(port(19423), otherTok), { stateDir: state });
	const server = identityServer(port(19422), {
		token,
		runDir,
		projectDir: proj,
	});
	const competitor = identityServer(port(19423), {
		token: otherTok,
		runDir: otherRun,
		projectDir: other,
	});
...
test("a dangling pointer reaped in the same pass does not disturb the healthy candidate", async () => {
	...
	await writeRunfile(healthyRun, RUN(port(19427), token), { stateDir: state });
	const server = identityServer(port(19427), {
		token,
		runDir: healthyRun,
	...
	try {
		const res = await resolveInstance({
			cwd,
			stateDir: state,
			selfHealProbePort: port(19429), // silent: the weird pointer takes the reap path
		});
...
test("row 10 in cwd: a foreign-host runfile is inert — never reclaimed, never a candidate, and it marks foreignSeen", async () => {
	...
	await writeRunfile(
		runDir,
		{
			...RUN(port(19314), "c2".repeat(16), "some-other-machine"),
...
test("row 10 via the registry: a locally-recorded runfile written on another host is inert too", async () => {
	...
	await writeRunfile(
		runDir,
		{
			...RUN(port(19315), "c3".repeat(16), "some-other-machine"),
...
test("a skipped instance is named by its boot file, and by the run dir's parent when that file cannot name it", async () => {
	...
	await writeRunfile(
		runDir,
		{ ...RUN(port(19313), "c1".repeat(16)), pid: dead.pid ?? 4_193_992 },
		{ stateDir: state },
	);
...
test("a token mismatch is never excused by a matching runDir (once a runfile carries a token, the token is the proof)", async () => {
	...
	const runDir = join(cwd, ".offbook");
	await writeRunfile(runDir, RUN(port(19300), "c4".repeat(16)), {
		stateDir: state,
	});
	...
	const server = identityServer(port(19300), {
		token: "c5".repeat(16),
		runDir,
		projectDir: cwd,
	});
...
test("--run-dir uses the dir's OWN runfile, never a nested .offbook, when both exist", async () => {
	...
	await writeRunfile(
		outer,
		{ ...RUN(port(19316), "c6".repeat(16)), pid },
		{
			stateDir: state,
		},
	);
	await writeRunfile(
		inner,
		{ ...RUN(port(19317), "c7".repeat(16)), pid },
		{
			stateDir: state,
		},
	);
	...
	expect(res.skipped[0]?.ctrlPort).toBe(port(19316));
	rmSync(state, { recursive: true, force: true });
...
test("a silent cwd instance never hides the registry's live one", async () => {
	...
	await writeRunfile(cwdRun, RUN(port(19301), "c8".repeat(16)), {
		stateDir: state,
	});
	const liveProj = scratch("offbook-res-livereg-");
	const liveRun = join(liveProj, ".offbook");
	const token = "c9".repeat(16);
	await writeRunfile(liveRun, RUN(port(19302), token), { stateDir: state });
	const server = identityServer(port(19302), {
		token,
		runDir: liveRun,
		projectDir: liveProj,
	});
...
test("row 9 never heals from an identity naming another runDir or another host — it reaps instead", async () => {
	...
	const aRun = join(aProj, ".offbook");
	await writeRunfile(aRun, RUN(port(19303), "d1".repeat(16)), {
		stateDir: state,
	});
	...
	let server = identityServer(port(19303), {
		token: "d1".repeat(16),
	...
	const bRun = join(bProj, ".offbook");
	await writeRunfile(bRun, RUN(port(19304), "d2".repeat(16)), {
		stateDir: state,
	});
	rmSync(join(bRun, "offbook.run"));
	server = identityServer(port(19304), {
		token: "d2".repeat(16),
		runDir: bRun,
		projectDir: bProj,
...
test("a healed instance is re-recorded under its canonical pointer name", async () => {
	...
	const server = identityServer(port(19305), {
		token,
		runDir,
		projectDir: proj,
	});
...
test("a pointer another process removed mid-scan is not reported as cleaned up", async () => {
	...
	const runDir = join(proj, ".offbook");
	await writeRunfile(runDir, RUN(port(19306), "d4".repeat(16)), {
		stateDir: state,
	});
...
test("tiebreak: the served projectDir decides, not the run dir's parent", async () => {
	...
	await writeRunfile(runDir, RUN(port(19307), token), { stateDir: state });
	const other = scratch("offbook-res-projofother-");
	const otherRun = join(other, ".offbook");
	const otherTok = "d6".repeat(16);
	await writeRunfile(otherRun, RUN(port(19308), otherTok), { stateDir: state });
	const srv = identityServer(port(19307), { token, runDir, projectDir: proj });
	const otherSrv = identityServer(port(19308), {
		token: otherTok,
		runDir: otherRun,
		projectDir: other,
	});
...
test("tiebreak: two live instances that both contain cwd stay ambiguous — stage 2 takes strict descendants only", async () => {
	...
	await writeRunfile(innerRun, RUN(port(19309), innerTok), { stateDir: state });
	await writeRunfile(outerRun, RUN(port(19310), outerTok), { stateDir: state });
	const innerSrv = identityServer(port(19309), {
		token: innerTok,
		runDir: innerRun,
		projectDir: cwd, // equal to cwd
	});
	const outerSrv = identityServer(port(19310), {
		token: outerTok,
		runDir: outerRun,
...
test("attributeCtrlPort attributes nothing to a foreign-host answer, nor to a runDir holding no runfile", async () => {
	...
	try {
		expect(await attributeCtrlPort(port(19311))).toBeUndefined();
	} finally {
		foreign.stop();
	}
	// the claimed run dir holds no runfile at all: nothing proves the claim
	const orphan = identityServer(port(19312), {
		token,
		runDir: join(proj, "no-such-run-dir"),
		projectDir: proj,
	});
	try {
		expect(await attributeCtrlPort(port(19312))).toBeUndefined();
	} finally {
		orphan.stop();
		rmSync(state, { recursive: true, force: true });
		rmSync(proj, { recursive: true, force: true });
