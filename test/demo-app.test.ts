...
test("proxy: /v1 pass-through when offbook is up; 502 when unreachable; /spike/fingerprint 200/404", async () => {
	// Port BASES: compose on ws 19112 / tcp 12993 / ctrl 19893, demo-app on
	// 19991 and 19992, plus the deliberately DEAD ctrl 19899 behind the 502
	// case. They are base literals, not necessarily the bound ports —
	// test/ports.ts maps each into this process's band (band 0 is the identity
	// map, so a plain local run binds these very numbers). Distinct bases never
	// share a slot inside a band, so 19899 stays dead in every band.
	const config = loadConfig({
		brokerWsPort: port(19112),
		brokerTcpPort: port(12993),
		controlPlanePort: port(19893),
	});
	...
	try {
		const mode = (await (
			await fetch(`http://localhost:${port(19991)}/v1/mode`)
		).json()) as { mode: string };
		expect(mode.mode).toBe("autonomous");

		const dead = await fetch(`http://localhost:${port(19992)}/v1/mode`);
		expect(dead.status).toBe(502);
		expect(await dead.json()).toEqual({ error: "offbook-unreachable" });

		const fp = await fetch(
			`http://localhost:${port(19991)}/spike/fingerprint?clientId=demo-app-x1`,
		);
		...
		const none = await fetch(
			`http://localhost:${port(19991)}/spike/fingerprint?clientId=nobody`,
		);
		expect(none.status).toBe(404);
		expect(await none.json()).toEqual({ error: "no-fingerprint" });

		const index = await fetch(`http://localhost:${port(19991)}/`);
		expect(index.status).toBe(200);
		expect(await index.text()).toContain('<div id="root">');

		const posted = await fetch(`http://localhost:${port(19991)}/v1/publish`, {
			method: "POST",
...
test("static routes: /, /main.js, and the catch-all 404", async () => {
	...
	try {
		expect(
			await (await fetch(`http://localhost:${port(19993)}/`)).text(),
		).toContain('<div id="root">');
		const js = await fetch(`http://localhost:${port(19993)}/main.js`);
		expect(js.status).toBe(200);
		expect(await js.text()).toBe("// stub bundle");
		expect((await fetch(`http://localhost:${port(19993)}/nope`)).status).toBe(
			404,
		);
	} finally {
		server.stop(true);
